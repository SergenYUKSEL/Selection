    ...
    {    
      ...
      if ($pseudolength <= 255)
      {
        if($mail == $mail2)
        {
          ...
          {
            $reqmail = $bdd->prepare("SELECT * FROM administrateurs WHERE mail = ? ");
            ...
            if($mailexist == 0)
            {
              if($passwd == $passwd2)
              {
                $insertmbr = $bdd->prepare("INSERT INTO administrateurs(pseudo, mail, password) VALUES(?, ?, ?)");
...
<html>
  <head>
    <title>Formulaire d'inscription Administrateur</title>
